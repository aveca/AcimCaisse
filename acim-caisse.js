...
function _broadcastNew(item,total){
  ...
  function _getTX(){return _mob?0.82:0.55;}  // overlay width: 18% mobile, 45% desktop
  function _getHY(){return _mob?8:120;}        // overlay start Y
  function _getLH(){return _mob?36:48;}        // line height
  ...
  function _createOverlay(){
    ...
    if(_mob){
      ...
      ov.style.cssText="position:fixed;bottom:0;left:0;right:0;height:40vh;z-index:1000000;pointer-events:none;background:#f8f8f8;border-top:1px solid rgba(230,81,0,0.3);overflow-y:auto;overflow-x:hidden;";
    }else{
      // Desktop: side panel on right
      ov.style.cssText="position:fixed;top:0;right:0;bottom:0;width:"+((1-tx)*100)+"%;z-index:1000000;background:#fff;pointer-events:auto;overflow-y:auto;";
    }
  ...
  function _pollCart(){
    ...
    _overlay.innerHTML="";
    _linesEls=[];
    var tx=_getTX(),hy=_getHY(),lh=_getLH();
    for(var i=0;i<info.length;i++){
      ...
      if(_mob){
        // Mobile: horizontal flow in bottom strip
        ln.style.cssText="position:relative;display:inline-flex;align-items:center;height:"+lh+"px;padding:4px 8px;margin:2px;cursor:pointer;pointer-events:auto;"+borderStyle+"transition:background 0.15s;border-radius:6px;white-space:nowrap;max-width:100%;overflow:hidden;text-overflow:ellipsis;font-size:13px;color:#333;background:#fff;";
        ...
        // Desktop: absolute positioned lines
        ln.style.cssText="position:absolute;left:0;right:0;height:"+lh+"px;top:"+String(hy+i*lh)+"px;cursor:pointer;pointer-events:auto;"+borderStyle+"transition:background 0.15s;";
        // Badge ✏️ pour prix=0
        if(isZero){
          var badge=document.createElement("span");
          badge.style.cssText="position:absolute;left:8px;top:50%;transform:translateY(-50%);font-size:12px;color:#e65100;";
          badge.textContent="✏️";
  ...
  function _inlineEdit(idx,clickX,clickY){
    ...
    if(_mob){
      ...
      var ticketLeft=Math.round(window.innerWidth*_getTX());
      var left=Math.min(Math.max(clickX-20,ticketLeft),window.innerWidth-cardW-10);
    ...
    var kh=function(e){
      ...
      if(e.key==="Tab"&&info.length>1){e.preventDefault();var ni2=e.shiftKey?(idx-1+info.length)%info.length:(idx+1)%info.length;card.remove();
        var nextTop=_getHY()+ni2*_getLH()+_getLH()/2;_inlineEdit(ni2,window.innerWidth*0.75,nextTop);}
    };
  ...
  document.addEventListener("keydown",function(e){
    ...
    if(e.key==="F2"){e.preventDefault();var info=_cartInfo();if(info.length&&!_dialogOpen())_inlineEdit(0,window.innerWidth*0.75,_getHY()+_getLH()/2);}
  });
